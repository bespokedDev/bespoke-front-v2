...
export default function StudentDetailPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({
        ...
        notes:
          updatedStudent.notes?.map((note: Note) => ({
            ...note,
            date: note.date ? extractDatePart(note.date) : "",
          })) || [],
  ...
  const handleViewEnrollment = async (enrollmentId: string) => {
    try {
      const enrollmentData = await apiClient(
  ...
  return (
    <div className="space-y-6">
      ...
      {isProfessor ? (
        // Vista de Profesor: Con tabs
        <Tabs defaultValue="profile" className="space-y-4">
          <TabsList>
            <TabsTrigger value="profile">Profile</TabsTrigger>
            <TabsTrigger value="canvaDocs">Student&apos;s Links</TabsTrigger>
          </TabsList>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm">
                      {student.dob ? formatDateForDisplay(student.dob) : "N/A"}
                    </p>
                  </div>
                  <div className="space-y-2">
                    <Label className="font-semibold flex items-center gap-2">
        ...
        >
          <TabsList>
            {isStudent ? (
              <>
                ...
                <TabsTrigger value="canvaDocs">My Links</TabsTrigger>
                {isAdmin && (
          ...
          <TabsContent value="enrollments">
            ...
            {(isAdmin || isStudent) &&
              studentInfo?.incomeHistory &&
              studentInfo.incomeHistory.length > 0 && (
                <Card className="mt-6">
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                  ...
                  <CardContent>
                    <div className="space-y-6">
                      {studentInfo.incomeHistory.map(
                        (incomeGroup, groupIndex) => (
                            ...
                            <div className="mb-4 pb-2 border-b">
                              ...
                              <div className="text-sm text-muted-foreground">
                                {formatDateForDisplay(
                                  incomeGroup.enrollment.startDate
                                )}{" "}
                                -{" "}
                                {formatDateForDisplay(
                                  incomeGroup.enrollment.endDate
                                )}
                              </div>
                            </div>
                                ...
                                <TableBody>
                                  {incomeGroup.incomes.map((income) => (
                                    <TableRow key={income._id}>
                                      ...
                                      <TableCell>
                                        {income.idDivisa.name}
                                      </TableCell>
                                      <TableCell>
                                        {income.idPaymentMethod.name}
                                      </TableCell>
              ...
              <CardContent className="pt-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-2xl font-bold text-secondary">
                      {studentInfo
                        ? `$${studentInfo.totalBalancePerClass.toFixed(2)}`
                        : "$0.00"}
                    </p>
                  </div>
                </div>
                ...
                {studentInfo?.enrollmentStatistics &&
                  studentInfo.enrollmentStatistics.length > 0 && (
                    <div className="mt-8 pt-8 border-t">
                      <h3 className="text-lg font-semibold mb-4">
                        Statistics by Enrollment
                      </h3>
                      <div className="space-y-6">
                        {studentInfo.enrollmentStatistics.map((stat, index) => (
                          <Card
                            key={stat.enrollmentId || index}
                            className="bg-muted/30"
                          >
                            <CardHeader>
                              <CardTitle className="text-base">
                                {stat.enrollmentInfo.planName} (
                                {stat.enrollmentInfo.enrollmentType})
                              </CardTitle>
                              <div className="text-sm text-muted-foreground">
                                {formatDateForDisplay(
                                  stat.enrollmentInfo.startDate
                                )}{" "}
                                -{" "}
                                {formatDateForDisplay(
                                  stat.enrollmentInfo.endDate
                                )}
                              </div>
                            </CardHeader>
                            <CardContent>
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                ...
                                {stat.rescheduleTime && (
                                  <div className="space-y-1">
                                    ...
                                    <p className="font-semibold">
                                      {stat.rescheduleTime.totalAvailableHours.toFixed(
                                        2
                                      )}{" "}
                                      hrs
                                    </p>
                                ...
                                {!isStudent &&
                                  stat.noShowClasses &&
                                  stat.noShowClasses.total > 0 && (
                                    <div className="space-y-1">
                                      <Label className="text-xs text-muted-foreground text-orange-600">
                                        No Show Classes
                                      </Label>
                                      <p className="font-semibold text-orange-600">
                                        {stat.noShowClasses.total}
                                      </p>
                                    </div>
                                  )}
                                {isAdmin &&
                                  stat.lostClasses &&
                                  stat.lostClasses.total > 0 && (
                                    <div className="space-y-1">
                                      <Label className="text-xs text-muted-foreground text-red-600">
                                        Lost Classes
                                      </Label>
                                      <p className="font-semibold text-red-600">
                                        {stat.lostClasses.total}
                                      </p>
                                    </div>
                                  )}
                              </div>
